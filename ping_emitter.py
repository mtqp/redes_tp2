...
class TraceRoute:
    ...
    def process(self, steps):
        ...
        total_avg_rtt = sum(all_rtts) / float(len(all_rtts)) 
...
class Step:
    ...
    def avg_route_rtt(self, total_avg_rtt):
        self.total_avg_rtt = total_avg_rtt
    ...
    def zrtt(self):
        if self._is_uknown_ip():
            return 'Unkwown'            
        return (self.rtt() - self.total_avg_rtt) / self.std()
    ...
    def __str__(self):
        ...
        return "TTL: {0}\tAVG: {1}\tZRTT: {2}\tIPs: {3}".format(ttl, rtt, zrtt, ips)
